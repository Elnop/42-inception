...
if [ -d "/run/mysqld" ]; then
	echo "Setup.sh : mysqld already present, skipping creation"
	chown -R mysql:mysql /run/mysqld
else
	echo "Setup.sh : mysqld not found, creating...."
	mkdir -p /run/mysqld
fi
...
EOF
    if [ "$MYSQL_DATABASE" != "" ]; then
		...
		if [ "$MYSQL_CHARSET" != "" ] && [ "$MYSQL_COLLATION" != "" ]; then
			echo "Setup.sh : $MYSQL_DATABASE : character set [$MYSQL_CHARSET] | collation [$MYSQL_COLLATION]"
			echo "CREATE DATABASE IF NOT EXISTS \`$MYSQL_DATABASE\` CHARACTER SET $MYSQL_CHARSET COLLATE $MYSQL_COLLATION;" >> $tfile
		else
			echo "Setup.sh : with character set: 'utf8' and collation: 'utf8_general_ci'"
